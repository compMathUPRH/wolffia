...
"""
    ...
    (On last names: Most hispssc people, Puerto Ricans included, use two surnames; 
...
from ui_pssEditor import Ui_PSSEditor
...
class PSSBuilder(QtGui.QDialog):
	...
	def __init__(self, appState, parent=None, settings=None):
		...
		self.ui.PSSdiagram.setPixmap(QtGui.QPixmap(QtCore.QString.fromUtf8(image)))
	...
	def on_saveButton_pressed(self):
		from PSSSaveDialog import PSSSaveDialog

		# generate tube
		self.generatePolymer()

		ntDialog = PSSSaveDialog(self.pss,files=self.files, parent=self)
		ntDialog.show()
		ntDialog.exec_()
...
if __name__ == '__main__':
	...
	load_gui = PSSBuilder(NanoCADState())
	load_gui.show()
	sys.exit(app.exec_())
