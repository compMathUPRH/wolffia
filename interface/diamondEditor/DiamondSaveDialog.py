# -*- coding: utf-8 -*-
#---------------------------------------------------------------------------
#  NanotubeSaveDialog.py
#  Version 0.1, October, 2011
#
#  Part of the single-wall nanotube builder.
...
class NanotubeSaveDialog(QtGui.QDialog):
	# Class Fields:
	# ui: stores reference tu user interface
	# pdbFile: string, PDF filename
	# tubes: Mixture, a Tube

	def __init__(self,tubes,files=None, parent=None):
		super(NanotubeSaveDialog, self).__init__(parent, modal = 1)

		self.ui = Ui_NanotubeSaveDialog()
	...
	def on_okButton_pressed(self):
		...
		if pdbFile == '':
			...
			#print "CNT(", self.n, ", ", self.m, ", ", self.tubeLength 
			#tube = Tube(self.n, self.m, self.tubeLength)
			self.tubes.writePDB(pdbFile)
			psfFile = str(self.ui.psfFilename.text())
			if psfFile != '':
				self.tubes.writePSF(psfFile)
			self.close()
