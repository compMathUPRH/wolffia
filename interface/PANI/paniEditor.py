...
from ui_paniEditor import Ui_PANIEditor
...
class PANIBuilder(QtGui.QDialog):
	...
	def __init__(self, appState, parent=None, settings=None):
		super(PANIBuilder, self).__init__(parent, modal=1)

		self.state = appState

		self.ui = Ui_PANIEditor()
		self.ui.setupUi(self)

		image = os.path.dirname(os.path.realpath(__file__))+"/Polyaniline_structure.png"
		self.ui.PANIdiagram.setPixmap(QtGui.QPixmap(QtCore.QString.fromUtf8(image)))

		self.history = History()
		self.isAdded = False

		self.paniPreview = MixtureViewer(self.history, self, None)
		self.ui.viewerLayout.addWidget(self.paniPreview)
		if settings != None:
			self.paniPreview.setHighResolution(settings.highResolution)
			self.paniPreview.setLabeling(settings.showLabels)
			self.paniPreview.showAxes(settings.showAxes)
			self.paniPreview.showHelp(settings.showHelp)
	...
	def generatePolymer(self):
		...
		self.history.currentState().addMolecule(PANI(n, m, monomers))
		self.paniPreview.update()
	...
	def on_mSpinBox_valueChanged(self,val):
		if self.ui.leucoemeraldineButton.isChecked():
			self.ui.nSpinBox.setValue(0)
		self.generatePolymer()

	def on_nSpinBox_valueChanged(self,val):
		if self.ui.emeraldineButton.isChecked():
	...
	def on_saveButton_pressed(self):
		from PANISaveDialog import PANISaveDialog

		# generate tube
		self.generatePolymer()

		ntDialog = PANISaveDialog(self.pani,files=self.files, parent=self)
		ntDialog.show()
		ntDialog.exec_()
...
if __name__ == '__main__':
	...
	load_gui = PANIBuilder(NanoCADState())
	load_gui.show()
	sys.exit(app.exec_())
